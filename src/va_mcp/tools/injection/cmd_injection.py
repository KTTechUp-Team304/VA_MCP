...
# safe_mode=True일 때 사용하는 시간 기반 페이로드 (데이터 유출 없음)
SAFE_PAYLOADS = [
    "; sleep 5",
    "| sleep 5",
    "$(sleep 5)",
...
class CmdInjectionTool(BaseTool):
    ...
    def run(self, tool_input: ToolInput) -> ToolResult:
        ...
        try:
            ...
            # safe_mode에 따라 페이로드 세트 선택
            if safe_mode:
                payload_list = extra.get("payload_list", SAFE_PAYLOADS)
            else:
                                ...
                                evidences.append(
                                    Evidence(
                                        ...
                                        note=(
                                            ...
                                            f"응답 지연 {elapsed:.1f}초 감지 "
                                            f"(임계값: {SAFE_MODE_THRESHOLD_SEC}초)"
                                        ),
                                    )
                                )
                        else:
            ...
            if evidences:
                ...
                return ToolResult(
                    ...
                    confidence=Confidence.HIGH if not safe_mode else Confidence.MEDIUM,
                    title=(
                        "OS Command Injection 취약점 발견 (Time-based)"
                        if safe_mode
                        else "OS Command Injection 취약점 발견"
                    ),
                    description=(
                        f"총 {len(evidences)}건의 명령어 인젝션 징후가 감지되었습니다. "
                        f"테스트 파라미터: {test_params}"
                        f"{' (safe_mode: 시간 기반 탐지)' if safe_mode else ''}"
                    ),
                    owasp=["A05:2025 Injection"],
                    cwe=["CWE-78"],
                    evidence=evidences,
